...
final class VehicleCostAllocationRepairFundLifecycleTest extends TestCase
{
    use RefreshDatabase;

    public function test_repair_fund_is_reserved_idempotently_without_financial_execution(): void
    {
        $o = Organization::query()->create(['name' => 'S065 master', 'type' => Organization::TYPE_MASTER, 'status' => Organization::STATUS_ACTIVE]);
        ...
        $v = Vehicle::query()->create(['registration_number' => 'S065', 'vin' => 'S065'.Str::upper(Str::random(13)), 'manufacturer' => 'Test', 'model' => 'Fund', 'mileage' => 0, 'odometer_unit' => 'km', 'lifecycle_status' => 'active', 'current_revision' => 1, 'active' => true]);
        VehicleOwnership::query()->create(['change_reason' => 'Initial S065 ownership.', 'public_id' => (string) Str::uuid(), 'vehicle_id' => $v->id, 'organization_context_id' => $o->id, 'owner_type' => 'organization', 'owner_organization_id' => $o->id, 'ownership_share_basis_points' => 10000, 'valid_from' => '2025-01-01', 'verification_status' => 'verified', 'recorded_by_user_id' => $u->id, 'revision' => 1]);
        $application = app(VehicleCostAllocationApplicationService::class);
        $draft = $application->create(['vehicle_id' => $v->id, 'source_type' => 'incident', 'occurred_on' => '2026-09-08', 'description' => 'Repair reserve.', 'currency' => 'CZK', 'lines' => [['cost_component' => 'damage', 'responsible_party_type' => 'organization', 'responsible_organization_id' => $o->id, 'net_amount' => '1000.00', 'vat_amount' => '210.00', 'gross_amount' => '1210.00', 'settlement_mode' => 'repair_fund_reserve', 'vat_treatment' => 'standard_rate', 'vat_rate_basis_points' => 2100]]], (int) $o->id, $u);
        ...
        $input = ['expected_instruction_revision' => 1, 'idempotency_key' => (string) Str::uuid(), 'reserve_purpose' => 'vehicle_repair', 'evidence_note' => 'Reservation evidence only.'];
        $service = app(VehicleCostAllocationRepairFundService::class);
        $first = $service->reserve($instruction['public_id'], $input, (int) $o->id, $u);
        $again = $service->reserve($instruction['public_id'], $input, (int) $o->id, $u);
        self::assertSame($first['reservation_public_id'], $again['reservation_public_id']);
        self::assertSame('reserved', $first['status']);
        self::assertFalse($first['invoice_created']);
        self::assertFalse($first['bank_transaction_created']);
        self::assertFalse($first['payment_marked']);
        self::assertFalse($first['fund_movement_created']);
        self::assertFalse($first['settlement_deduction_applied']);
        self::assertDatabaseCount('vehicle_cost_allocation_repair_fund_reservations', 1);
        self::assertDatabaseCount('vehicle_cost_allocation_repair_fund_events', 1);
